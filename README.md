...
| `COMMUNITY_APPEND_ENABLED` | `false` | 本期已投注（`status.my_bet` 存在）时：`false`=跳过重复投注（防手动重跑重复烧分/换号被拒）；`true`=追加到**同一组已选号码** |
| `MONOPOLY_COOLDOWN_MS` | `6000` | 大富翁遇服务端冷却时的重试等待（ms）。总预算 = `COOLDOWN_MS×MAX_ATTEMPTS`；设 `0` 遇冷却直接停 |
